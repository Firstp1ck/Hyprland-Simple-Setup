...
grep -Fq 'Select the language setting for all selected shells (fish):' <<<"$output"
grep -Fxq 'set -gx LANG "en_US.UTF-8"' "$HOME/.config/fish/conf.d/01-env.fish"
...
jq -e '
  ...
  and .roles.file_manager.package == "thunar"
  and ([.selected.file_manager[].package] == ["thunar"])
  and .roles.tui_file_manager.package == "mc"
  and ([.selected.tui_file_manager[].package] == ["yazi", "ranger", "mc"])
...
output=$(printf '1\n' | NON_INTERACTIVE=false "$repo_root/setup.sh" --test-scenario roles)
