#!/usr/bin/env fish
# release.fish - Automated version release script for Hyprland-Simple-Setup
#
...
#   3. PKGBUILD updates (AUR -git only)
#   4. Build and GitHub release
#   5. AUR publishing (AUR -git only)
...
function phase3_pkgbuild_updates
    ...
    # This repo only has an AUR -git package (no -bin variation).
    # For -git PKGBUILDs, pkgver is typically computed via pkgver() and not pinned.
    # We keep this step limited to a safe pkgrel reset (when present).
    log_step "Update hyprland-simple-setup-git PKGBUILD"
...
function phase4_build_release
    ...
    # Step 4.2: Commit and push all changes
...
function phase5_aur_update
    ...
    # This repo only has an AUR -git package (no -bin variation).
    # Step 5.1: Push hyprland-simple-setup-git to AUR
    log_step "Pushing hyprland-simple-setup-git to AUR"
    ...
    if test "$DRY_RUN" = true
        ...
        if test $status -eq 0
            log_success "Pushed hyprland-simple-setup-git to AUR"
        else
...
function main
    ...
    _bold_cyan; echo "║           HYPRLAND-SIMPLE-SETUP RELEASE AUTOMATION                     ║"; _reset
    _bold_cyan; echo "╚════════════════════════════════════════════════════════════════════════╝"; _reset
    echo
    ...
    echo "  • GitHub release: https://github.com/Firstp1ck/Hyprland-Simple-Setup/releases"
    echo "  • Release includes the correct tag and notes (source archives are provided by GitHub)"
    echo "  • AUR package hyprland-simple-setup-git is updated"
    echo
